...
4. [Usage](#usage)
5. [Competency Questions](#competency-questions)
6. [Future Work](#future-work)
7. [Sources and Licensing](#sources-and-licensing)
8. [Citation](#citation)
...
## Statistics
Figures below are from the accompanying paper (see [Citation](#citation)).

- 785 unique ATC drugs, 4,753 actively-connected SNOMED CT concepts (the full SNOMED CT hierarchy has 382,278 concepts in total, only used via `rdfs:subClassOf` for querying)
- 66 drug classes, ~12 members per class on average
- `AKIConcept`: 42 direct / 1,634 indirect members. `CKDConcept`: 199 direct / 6,168 indirect members
- 960 DDI types, of which 519 require an action and 76 are nephrotoxic (direct and indirect); 30 DDIs list reduced kidney function itself as a risk factor
- Edge counts by relation: `hasInteractionWith` 42,971 (symmetry inferred), `hasAdverseEvent` 37,659, `isIndicatedFor` 3,966, `isContraIndicatedFor` 3,535, `isRiskFactorFor` 108
...
**More instructions and examples for link prediction usage will follow soon!**
...
- **nephrotoxic_classes.ttls**: this contains the SNOMED CT concepts that fall under the NephrotoxicConcept class (AKIConcept and CKDConcept).
...
@article{vos2026computer,
  title={Computer-Interpretable Domain Knowledge for Drug-Induced Acute Kidney Injury: a Knowledge Graph Approach},
  author={Vos, Romy and Vagliano, Iacopo and Boersma, Cornelis and van Harmelen, Frank and Ten Teije, Annette and Klopotowska, Joanna E},
